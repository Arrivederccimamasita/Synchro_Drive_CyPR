...
switch (sel)
    ...
    selection='ï¿½Comenzar orientado hacia ese punto?:\n 0:No\n 1:Sï¿½\n';
    ...
    figure
    subplot(2,1,1);plot(t,posx);xlabel('Tiempo (s)');ylabel('Posiciï¿½n cartesiana X (m)');title('Posiciï¿½n cartesiana X frente al tiempo');grid;subplot(2,1,2);plot(t,posy);xlabel('Tiempo (s)');ylabel('Posiciï¿½n cartesiana Y (m)');title('Posiciï¿½n cartesiana Y frente al tiempo');grid;
    case 2
    % MOVER POR UNA RECTA
    % Posiciones iniciales del integrador
     pos_init=[0;0;0];
    ...
    subplot(2,1,1);plot(t,theta_d);xlabel('Tiempo (s)');ylabel('Velocidad de desplazamiento (rad/s)');title('Variación de la velocidad angular de desplazamiento a lo largo de la trayectoria');xlim([0 50]);grid;subplot(2,1,2);plot(t,omega);xlabel('Tiempo (s)');ylabel('Velocidad de rotación (rad/s)');title('Variación de la velocidad angular de rotación a lo largo de la trayectoria');xlim([0 50]);grid;
     ...
     % CASO, HE CREADO UN GENERADOR DE TRAYECTORIAS SIMPLÃ“N DE TODO.
%      Kv=0.5;Ki=0.01;Kh=2;
%      R=0.4;
    ...
    % No se gira un volante a mas de 10-15 deg/sec, por tanto, ahÃ­ estarÃ¡ la saturaciÃ³n del movimiento
     ...
     if sel==0
        ...
        figure;plot(P(1,:),P(2,:),'*r','LineWidth',2);hold on;plot(posx,posy,'b','LineWidth',1);grid;...
            title('Control de seguimieto a trayectoria por interpolación');xlabel('Posición cartesiana X');ylabel('Posición cartesiana Y');
        figure;subplot(2,1,1);plot(t,theta_d);xlabel('Tiempo (s)');ylabel('Velocidad de desplazamiento (rad/s)');...
